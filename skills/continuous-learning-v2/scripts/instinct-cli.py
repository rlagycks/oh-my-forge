...
def cmd_evolve(args) -> int:
    ...
    # Constraint candidates (failure instincts from /error-capture)
    failure_instincts = [i for i in instincts if i.get('outcome') == 'failure' and i.get('source') == 'error-capture']
    if failure_instincts:
        print(f"\n## CONSTRAINT CANDIDATES (from /error-capture) — {len(failure_instincts)}\n")
        print("  These failure instincts should be promoted to ontology constraints[]:")
        print()
        for inst in failure_instincts:
            domain = inst.get('linked_domain', '(no linked domain)')
            prevents_match = re.search(r'## Prevents\s*\n\s*(.+?)(?:\n\n|\n##|$)', inst.get('content', ''), re.DOTALL)
            prevents = prevents_match.group(1).strip() if prevents_match else inst.get('id', 'unnamed')
            print(f"  • {inst.get('id')} → {domain}")
            print(f"    Prevents: {prevents[:100]}")
            if inst.get('linked_domain'):
                print(f"    Action: add to {inst['linked_domain']}.constraints[] in .claude/ontology/index.json")
            print()
    ...
    if args.generate:
        ...
        generated = _generate_evolved(skill_candidates, workflow_instincts, agent_candidates, evolved_dir, failure_instincts)
        if generated:
...
def _generate_evolved(skill_candidates: list, workflow_instincts: list, agent_candidates: list, evolved_dir: Path, failure_instincts=None) -> list[str]:
    ...
    # Generate constraint proposals from failure instincts (source: error-capture, outcome: failure)
    if failure_instincts:
        proposals_dir = evolved_dir / "constraint-proposals"
        proposals_dir.mkdir(parents=True, exist_ok=True)
        proposal_file = proposals_dir / "pending-constraints.md"

        content = "# Pending Ontology Constraint Proposals\n\n"
        content += "Generated by `/evolve --generate` from failure instincts.\n"
        content += "Review and apply to `.claude/ontology/index.json` manually or via `/error-capture`.\n\n"

        for inst in failure_instincts:
            inst_id = inst.get('id', 'unnamed')
            linked_domain = inst.get('linked_domain', '')
            inst_content = inst.get('content', '')
            ...
            content += f"## {inst_id}\n\n"
            if linked_domain:
                content += f"**Target domain**: `{linked_domain}`\n\n"
            else:
                content += f"**Target domain**: (not linked — identify manually)\n\n"
            if prevents:
                content += f"**Proposed constraint**: {prevents}\n\n"
            if root_cause:
                content += f"**Root cause**: {root_cause}\n\n"
            content += "**Action**: Add to `constraints[]` in `.claude/ontology/index.json`\n\n"
            content += "---\n\n"

        proposal_file.write_text(content, encoding="utf-8")
        generated.append(str(proposal_file))
