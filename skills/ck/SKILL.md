...
Scripts live at: `${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}/skills/ck/commands/` (expand `~` with `$HOME`).
...
The hook at `${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}/skills/ck/hooks/session-start.mjs` must be registered in
...
```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat \"$HOME/.claude/.omf-root\" 2>/dev/null || echo \"$HOME/.claude\")}}/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
