...
oh-my-forge는 로컬 훅 텔레메트리를 별도의 호스팅 분석 서비스로 전송하지 않지만, 워크플로 연속성을 위해 일부 데이터를 `~/.claude/` 아래에 로컬로 저장합니다:

- 세션 요약과 후속 작업 메모
- 토큰/비용 메트릭
- `ECC_ENABLE_BASH_COMMAND_LOG=1`일 때만 저장되는, 민감값을 가린 Bash 명령 감사 로그

훅 처리는 Claude Code 내부에서 로컬로 실행됩니다.
다만 MCP 서버를 opt-in 하거나 외부 서비스를 호출하는 도구를 사용하면 네트워크 접근은 발생할 수 있습니다.
