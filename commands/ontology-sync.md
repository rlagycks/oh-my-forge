...
## Step 3b — Blast Radius 분석 (dependsOn 자동 추론)

`scripts/lib/ontology-blast-radius.js`의 `inferDependsOn(projectRoot, indexJson)` 함수를 호출하여 누락된 `dependsOn` 관계를 추론한다.

```js
const { inferDependsOn } = require('./scripts/lib/ontology-blast-radius');
const suggestions = inferDependsOn(projectRoot, indexJson);
```

추론 방식:
- **Pass 1 — 경로 중첩**: 한 도메인의 `files[]` 경로가 다른 도메인 경로의 하위 경로인 경우
- **Pass 2 — require() 스캔**: `files[]` 내 `.js` 파일을 읽어 `require('./...')` 상대 경로가 다른 도메인 파일을 참조하는지 확인

제안 사항이 있으면 출력하고 사용자에게 적용 여부를 묻는다:

```
Blast Radius 분석 결과:
  domain_session   → domain_state_store (require 감지: state-store/index.js)
  domain_hooks     → domain_common (files[] 경로 중첩)

  [A] dependsOn 자동 추가   [S] 건너뛰기
```

- `[A]` 선택 시: 해당 도메인의 `dependsOn` 배열에 새 항목을 추가한다 (중복 제외)
- `[S]` 선택 시: 변경 없이 다음 Step으로 진행한다
- `--check` 모드에서는 제안만 출력하고 적용하지 않는다
- `--fix` 모드에서는 사용자 확인 없이 자동으로 추가한다
- 새로운 제안이 없으면 이 단계를 조용히 건너뛴다
...
- 새 엔트리 필드 순서: `files`, `spec`, `detail`, `owner`, `symbols`, `dependsOn`, `codexWorkerHint`
  - `spec`: `docs/features/<name>.md` — 사람이 읽는 기능 문서 (CI 검증 대상)
  - `detail`: `.claude/ontology/domain_<name>.json` — AI용 상세 JSON (존재할 경우 연결, 선택)
...
if [ -f "$PLUGIN_ROOT/scripts/ci/validate-ontology.js" ]; then
  ...
  echo "CI 검증: SKIPPED (scripts/ci/validate-ontology.js 없음 — OMF 개발 레포 전용)"
fi
```
