...
2. `node "${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}/scripts/lib/ontology.js" query --domain <domain_id>`로 각 도메인의 `constraints[]`를 읽어 구현 전 체크리스트로 보관한다
