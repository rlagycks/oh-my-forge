...
- Phase 2 EXPLORE에서 코드베이스 탐색 전에 도메인 컨텍스트를 먼저 조회한다
- Phase 5 ARCHITECT에서 영향 받는 도메인의 `constraints[]`를 필수 제약으로 포함한다
- Phase 6 GENERATE 계획 문서에 `## Domain Context` 섹션을 추가한다
- 단일 도메인 구현이라면 `/codex-delegate`로 위임을 권장 메시지로 추가한다

인덱스가 없으면 아래 표준 절차를 그대로 실행한다.
...
### Ontology Query (Ontology Mode only)

온톨로지 인덱스가 있으면 코드베이스 탐색 전에 도메인 컨텍스트를 먼저 수집한다:

1. `index.json`에서 feature와 관련된 `domain_*` 키를 찾는다
2. 해당 도메인의 `files[]`, `symbols[]`, `constraints[]`, `dependsOn[]`를 읽는다
3. `spec` 필드가 있으면 해당 spec 파일을 읽어 `## 핵심 제약`을 추출한다
4. `dependsOn` 도메인들의 constraints도 추가로 수집한다

수집한 도메인 정보를 Unified Discovery Table에 **Domain** 행으로 포함시키고, Phase 5 설계 시 우선 반영한다.
...
## Domain Context (Ontology Mode only)

| Domain | Key Files | Constraints |
|--------|-----------|-------------|
| `domain_xxx` | `path/to/file.js` | 제약 1, 제약 2 |

> 단일 도메인 구현: `/codex-delegate domain_xxx "<task>"` 위임을 고려하라.
