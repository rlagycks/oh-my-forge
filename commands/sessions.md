...
const root = (process.env.CLAUDE_PLUGIN_ROOT||process.env.CODEX_PLUGIN_ROOT||(()=>{try{return require('fs').readFileSync(require('path').join(require('os').homedir(),'.claude','.omf-root'),'utf8').trim()}catch(e){return require('path').join(require('os').homedir(),'.claude')}})());
...
```bash
node -e "
const aa = require((process.env.CLAUDE_PLUGIN_ROOT||process.env.CODEX_PLUGIN_ROOT||(()=>{try{return require('fs').readFileSync(require('path').join(require('os').homedir(),'.claude','.omf-root'),'utf8').trim()}catch(e){return require('path').join(require('os').homedir(),'.claude')}})())+'/scripts/lib/session-aliases');
