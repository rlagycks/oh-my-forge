...
### Phase 1.5 — ONTOLOGY CONTEXT

변경된 파일 목록이 확인되면, `.claude/ontology/index.json`을 읽어 각 파일이 속한 도메인을 찾는다.

```bash
node "${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}/scripts/lib/ontology.js" query --file <changed_file> 2>/dev/null
```

파일이 `domain_*`의 `files[]`에 포함된 경우:
- 해당 도메인의 `constraints[]`를 Phase 2 리뷰 체크리스트에 추가한다
- `riskLevel: "high"` 도메인이면 CRITICAL 이슈로 분류 기준을 강화한다
- `dependsOn` 도메인이 있으면 연관 도메인 파일도 영향 여부를 확인한다

도메인 제약 위반은 **HIGH** 심각도로 리포트한다.
...
### Phase 2.5 — ONTOLOGY CONTEXT

변경 파일 목록이 확인되면, `.claude/ontology/index.json`을 읽어 각 파일의 도메인을 확인한다. 도메인 `constraints[]`를 Phase 3 리뷰에 포함시키고, 위반 시 HIGH로 분류한다.
...
| Check | Result |
|---|---|
| Type check | PASS: Pass / FAIL: Fail / SKIPPED: Skipped |
