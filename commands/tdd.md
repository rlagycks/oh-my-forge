...
0. **Query ontology** (if `.claude/ontology/index.json` exists) — 구현 대상 파일이 속한 도메인의 `models[]`와 `constraints[]`를 읽어 인터페이스 스캐폴딩과 테스트 픽스처에 활용한다. 도메인 `constraints[]`는 엣지 케이스 테스트 항목으로 자동 추가한다.
