...
0. **Query ontology** (if `.claude/ontology/index.json` exists) — `node "${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}/scripts/lib/ontology.js" query --domain <id> --fields models,constraints`로 `models[]`와 `constraints[]`를 읽어 인터페이스 스캐폴딩과 테스트 픽스처에 활용한다. 도메인 `constraints[]`는 엣지 케이스 테스트 항목으로 자동 추가한다.
