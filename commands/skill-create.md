...
생성된 스킬이 `.claude/ontology/index.json`의 특정 도메인 파일을 주로 다루는 경우:
- 해당 `domain_*` 키를 SKILL.md의 `## When to Use`에 명시한다
- 도메인 `constraints[]`를 스킬 제약 섹션에 포함시킨다
- 스킬이 새 도메인을 정의한다면 `docs/features/` spec 생성 후 `/ontology-sync`를 실행한다
...
- `/ontology-sync` - 새 스킬이 도메인을 정의하는 경우 인덱스 동기화
