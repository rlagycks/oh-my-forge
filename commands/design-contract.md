...
- Validate saved contracts before promotion with
  `PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-$(cat "$HOME/.claude/.omf-root" 2>/dev/null || echo "$HOME/.claude")}}"; node "$PLUGIN_ROOT/scripts/lib/design-contract.js" validate --dir "<contracts-dir>"`.
