...
Installs access to 36 agents, 144 skills, and 77 commands.
...
Domain JIT recall injects at most two non-expired, `validated` lessons (each
with two distinct evidence records); candidates are not injected. Reports link
injections to the latest episode outcome, but without `--recall-used true` the
link is observational—not a causal claim.
...
For performance or release claims, use `run-paired-benchmark.js` with isolation,
comparable configuration, and a failing-baseline gate. The adapter contract and
its `adapter_attested` limitation are documented in the
[evaluation guide](docs/evals/harness-effectiveness.md).
