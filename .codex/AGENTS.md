...
|-------|-------|-------------|
| `sandbox_mode = "read-only"` | OS-level | Blocks all filesystem writes outside allowed paths |
...
5. Use `sandbox_mode = "workspace-write"` for implementer, `"read-only"` for explorer/reviewer

## Implementer Role

The `implementer` agent role (`agents/implementer.toml`) is the ECC execution worker:
- `sandbox_mode = "workspace-write"` — can write files, run tests
- Receives structured BRIEFs from Claude via `/codex-delegate`
- Must report back in HANDOFF format: `RESULT / FILES CHANGED / TESTS / SUMMARY`
- Only modifies files explicitly listed in the BRIEF's FILES section
