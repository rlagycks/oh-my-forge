...
- 단, ontology observation spool은 일반 하네스 이벤트가 아니라 후보 유지보수용 입력 큐다. DB에는 원문이 아니라 `observation_id`, hash, 상대 경로, 후보 상태와 cursor만 저장한다. PostToolUse capture 훅은 DB를 절대 열지 않는다.
