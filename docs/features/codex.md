...
OpenAI Codex CLI를 ECC에 통합하여 Claude(오케스트레이터)와 Codex(구현자)의 하이브리드 멀티 에이전트 구조를 구성하는 시스템. `config.toml`이 Codex 행동을 제어(sandbox, approval, 역할별 지시)하고, `openai-codex` 플러그인(구: `codex-plugin-cc`)이 Claude-Codex 통신 채널을 제공한다. `merge-mcp-config.js`가 ECC MCP 서버 설정을 `~/.codex/config.toml`에 안전하게 병합한다.
...
- `scripts/lib/codex-handoff.js` — request/result schema 검증, BRIEF 생성, dispatch를 소유
- `scripts/lib/false-normal-detector.js` — completion report 파싱과 `RESULT:DONE` false-normal downgrade 판정의 단일 소유자
- `schemas/codex-handoff-*.schema.json` — Codex handoff request/result 계약
...
- `RESULT: DONE`은 `EVIDENCE`, `FALSE NORMAL CHECKS`, `FALSE NORMAL SIGNALS: none`, `NEXT ACTION`이 없으면 `BLOCKED`로 강등
- caller별 completion 파서는 만들지 않는다. Codex handoff, orchestration handoff 등 `RESULT / EVIDENCE / FALSE NORMAL ...` 형식을 판정하는 코드는 `false-normal-detector.js`를 재사용한다.
