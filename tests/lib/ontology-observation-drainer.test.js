...
async function main() {
  ...
  await test('applies the ontology candidate migration only once when reopening a state store', async () => {
    ...
    try {
      ...
      assert.deepStrictEqual(first.getAppliedMigrations().map(migration => migration.version), [1, 2, 3, 4]);
      first.close();
      const reopened = await createStateStore({ dbPath });
      assert.deepStrictEqual(reopened.getAppliedMigrations().map(migration => migration.version), [1, 2, 3, 4]);
      reopened.close();
    } finally {
      fs.rmSync(fixture.root, { recursive: true, force: true });
    }
